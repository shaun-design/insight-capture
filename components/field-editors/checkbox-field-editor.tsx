...
export function createDefaultCheckboxSettings(): CheckboxFieldSettings {
  return {
    prompt: "",
    options: [
...
function SortableOption({
  ...
  onUpdate: (id: string, field: keyof CheckboxOption, value: string) => void;
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-2 group/opt">
      ...
      <div className="h-4 w-4 shrink-0 rounded-sm border-2 border-muted-foreground/30" />
      <Input
        value={option.text}
...
export function CheckboxFieldEditor({
  ...
  isActive?: boolean;
  value: CheckboxFieldSettings;
  onChange: (next: CheckboxFieldSettings) => void;
}) {
  const settings = value;
  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 5 } }));

  function updateOption(id: string, field: keyof CheckboxOption, value: string) {
    onChange({
      ...settings,
  ...
  function set<K extends keyof CheckboxFieldSettings>(key: K, value: CheckboxFieldSettings[K]) {
      ...
      >
        <div className="overflow-hidden">
          <div className="flex flex-col gap-3">
            ...
            <div className={`flex gap-4 ${settings.displayOrientation === "Horizontal" ? "flex-row flex-wrap" : "flex-col"}`}>
              {(displayOptions.length > 0 ? displayOptions : settings.options).map((opt) => (
                <div key={opt.id} className="flex items-center gap-2">
                  <div className="h-4 w-4 shrink-0 rounded-sm border-2 border-muted-foreground/30" />
                  <span className={`text-sm ${opt.text ? "text-foreground" : "text-muted-foreground/40 italic"}`}>
                    {opt.text || "Add Response Text"}
                  </span>
                </div>
              ))}
            </div>
      ...
      >
        <div className="overflow-hidden">
          <div className="flex flex-col gap-5">
            ...
            <div className="w-fit">
              <Select
                value={settings.displayOrientation}
                onValueChange={(v) => set("displayOrientation", v as "Vertical" | "Horizontal")}
              >
                <SelectTrigger className="h-7 text-xs">
                  <span className="text-muted-foreground mr-1">Display:</span>
                  <SelectValue />
                </SelectTrigger>
